...
video_path: str = ""
score_endpoint: str = ""
score_http: httpx.AsyncClient = None
...
async def config():
    ...
    return {"dual_vlm": bool(score_endpoint)}
...
def main():
    global client, model_name, video_path, score_endpoint, score_http
    ...
    if args.score_endpoint:
        ...
        score_http = httpx.AsyncClient(timeout=httpx.Timeout(60.0))
        print(f"Dual VLM mode enabled")
        print(f"  Momentum endpoint: {args.endpoint}")
        print(f"  Score endpoint:    {score_endpoint}")
    else:
